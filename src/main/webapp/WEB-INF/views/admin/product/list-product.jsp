...
<body>
	<section class="content-header">
		<h1>
			Product List <small>Control panel</small>
		</h1>
		<ol class="breadcrumb">
							...
							<div class="row">
								<div class="col-sm-12">
									<table id="example1"
										...
										<tbody>
											<c:if test="${!empty productsPaginate}">
												<c:forEach var="product" items="${productsPaginate}" varStatus="loop">
													<tr>
														<td>${product.ID}</td>
														<td>
															<c:forEach var="cateogry" items="${listCategories}">
																<c:if test="${(cateogry.id) == (product.ID_CATEGORIES)}">
																	${cateogry.name}
																</c:if>
															</c:forEach>
														</td>
														...
														<td><a class="btn btn-primary btn-xs purple editView" title="Edit"
															href="<c:url value="/admin/edit/${product.ID}" />" ><i
																class="fa fa-edit"></i> Edit</a> 
															<a class="btn btn-danger btn-xs purple deleteView" title="Delete"
															href="<c:url value="/admin/delete/${product.ID}" />" ><i
							...
							<ul class="pagination">
								<c:forEach var="item" begin="1" end="${paginateInfo.totalPage}" varStatus="loop">
									<c:if test="${(loop.index) == paginateInfo.currentPage}">
										<li class="active">
			                                <a href="<c:url value="/admin/list/${loop.index}"/>">${loop.index}</a>
			                            </li>
									</c:if>
									<c:if test="${(loop.index) != paginateInfo.currentPage}">
										<li>

							...
							<div class="row">
								<div class="col-sm-12">
									<table id="example1"
										...
										<tbody>
											<c:if test="${!empty accountPaginate}">
												<c:forEach var="user" items="${accountPaginate}" varStatus="loop">
													<tr>
														<td>${user.id}</td>
														<td>${user.userName}</td>
														...
														<td><a class="btn btn-primary btn-xs purple editView" title="Edit"
															href="<c:url value="/admin/user/edit/${user.id}" />" ><i
																class="fa fa-edit"></i> Edit</a> 
															<a class="btn btn-danger btn-xs purple deleteView" title="Delete"
															href="<c:url value="/admin/user/delete/${user.id}" />" ><i
							...
							<ul class="pagination">
								<c:forEach var="item" begin="1" end="${paginateInfo.totalPage}" varStatus="loop">
									<c:if test="${(loop.index) == paginateInfo.currentPage}">
										<li class="active">
			                                <a href="<c:url value="/admin/user/list/${loop.index}"/>">${loop.index}</a>
			                            </li>
									</c:if>
									<c:if test="${(loop.index) != paginateInfo.currentPage}">
										<li>

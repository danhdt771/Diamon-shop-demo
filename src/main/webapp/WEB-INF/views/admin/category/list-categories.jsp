...
<body>
	<section class="content-header">
		<h1>
			Category List <small>Control panel</small>
		</h1>
		<ol class="breadcrumb">
									...
									<table id="example1"
										...
										<tbody>
											<c:if test="${!empty cateogriesPaginate}">
												<c:forEach var="category" items="${cateogriesPaginate}" varStatus="loop">
													<tr>
														<td>${category.id}</td>
														<td>${category.name}</td>
														<td><a class="btn btn-primary btn-xs purple editView" title="Edit"
															href="<c:url value="/admin/category/edit/${category.id}" />" ><i
																class="fa fa-edit"></i> Edit</a> 
															<a class="btn btn-danger btn-xs purple deleteView" title="Delete"
															href="<c:url value="/admin/category/delete/${category.id}" />" ><i
							...
							<ul class="pagination">
								<c:forEach var="item" begin="1" end="${paginateInfo.totalPage}" varStatus="loop">
									<c:if test="${(loop.index) == paginateInfo.currentPage}">
										<li class="active">
			                                <a href="<c:url value="/admin/category/list/${loop.index}"/>">${loop.index}</a>
			                            </li>
									</c:if>
									<c:if test="${(loop.index) != paginateInfo.currentPage}">
										<li>
